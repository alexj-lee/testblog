...
{#if Object.values($tocStore.items).length && Object.values($tocStore.items).length > 1}
	<section
		class="fixed right-4 bottom-1 max-w-[12em] 
		rounded-xl bg-white/25 p-2 backdrop-blur hover:bg-white/30"
	>
		{#if !isOpen}
			<button class="z-50 flex items-center justify-center" on:click={() => (isOpen = !isOpen)}>
